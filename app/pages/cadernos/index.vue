      ...
      <div class="p-3 sm:p-4 border-b border-base">
        ...
        <div class="flex items-center gap-2">
          <div class="relative flex-1">
            ...
            <div v-if="showAddMenu" class="absolute right-0 top-full mt-1 w-52 bg-[var(--bg-card)] border border-base rounded-xl shadow-lg py-1 z-20">
              <button class="w-full text-left px-3 py-2 text-small text-base-primary hover:bg-surface-secondary transition-colors" @click="showAddMenu = false; openCreate(null)">
                Novo caderno
              </button>
...
// Detect if current note is being generated by AI (read-only + auto-refresh)
const noteIsGenerating = computed(() => {
  if (!editingNote.value?.source_document_id) return false
  const doc = docStore.documents.find(d => d.id === editingNote.value?.source_document_id)
